...
@dataclass
class CDN(HelmPackage):

    name: str = "cdn-backend"
    namespace: str = "prod"
    icon: str = "/api/youwol-infra/cdn/icon"

    chart_folder: Path = Configuration.services_folder / "cdn-backend" / "chart"
    with_values: dict = field(default_factory=lambda: {})

    secrets: dict = field(default_factory=lambda: {
        "youwol-auth": Configuration.secrets_folder / "youwol-auth.yaml"
        })
...
async def ws_endpoint(ws: WebSocket):
    ...
    await ws.accept()
    WebSocketsStore.cdn = ws
    await WebSocketsStore.cdn.send_json({})
...
async def send_status(
    ...
    cdn = next(p for p in config.deployment_configuration.packages
               if p.name == CDN.name and p.namespace == namespace)

    is_installed = await cdn.is_installed()
    if not is_installed:
        resp = Status(
        ...
        await WebSocketsStore.cdn.send_json(to_json_response(resp))
        return
...
@router.get("/icon")
async def icon():
    path = Path(__file__).parent / 'cdn.png'
    return FileResponse(path)


@router.get("/{namespace}/status", summary="trigger fetching status of CDN component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of CDN component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
    ...
    await install_pack(
        ...
        channel_ws=WebSocketsStore.cdn,
        finally_action=lambda: status(request, namespace, config)
        )


@router.post("/{namespace}/upgrade", summary="trigger upgrade of CDN component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
