...
@dataclass
class TreedbBackend(HelmPackage):

    name: str = "treedb-backend"
    namespace: str = "prod"
    icon: str = "/api/youwol-infra/treedb-backend/icon"

    chart_folder: Path = Configuration.services_folder / "treedb-backend" / "chart"
...
async def send_status(
    ...
    treedb = next(p for p in config.deployment_configuration.packages
                  if p.name == TreedbBackend.name and p.namespace == namespace)

    is_installed = await treedb.is_installed()
    if not is_installed:
        resp = Status(
...
@router.get("/{namespace}/status", summary="trigger fetching status of TreeDb component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of TreeDb component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of TreeDb component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
