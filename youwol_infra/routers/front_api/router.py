...
async def send_status(
    ...
    front_api = next(p for p in config.deployment_configuration.packages
                     if p.name == FrontApi.name and p.namespace == namespace)

    is_installed = await front_api.is_installed()
    if not is_installed:
        resp = Status(
...
@router.get("/icon")
async def icon():
    path = Path(__file__).parent / 'login.png'
    return FileResponse(path)


@router.get("/{namespace}/status", summary="trigger fetching status of front-api component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of front-api component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of front-api component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
