...
@dataclass
class StoriesBackend(HelmPackage):

    name: str = "stories-backend"
    namespace: str = "prod"
...
async def send_status(
    ...
    story = next(p for p in config.deployment_configuration.packages
                 if p.name == StoriesBackend.name and p.namespace == namespace)

    is_installed = await story.is_installed()
    if not is_installed:
        resp = Status(
...
@router.get("/icon")
async def icon():
    path = Path(__file__).parent / 'stories.png'
    return FileResponse(path)


@router.get("/{namespace}/status", summary="trigger fetching status of stories-backend component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of stories-backend component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of stories-backend component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
