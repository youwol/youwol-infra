...
@dataclass
class FluxRunner(HelmPackage):

    name: str = "flux-runner"
    namespace: str = "prod"
...
async def send_status(
    ...
    flux = next(p for p in config.deployment_configuration.packages
                if p.name == FluxRunner.name and p.namespace == namespace)
...
@router.get("/{namespace}/status", summary="trigger fetching status of Flux-runner component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of Flux-runner component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of Flux-runner component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
