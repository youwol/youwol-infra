...
@dataclass
class FluxBackend(HelmPackage):

    name: str = "flux-backend"
    namespace: str = "prod"
...
async def send_status(
    ...
    flux = next(p for p in config.deployment_configuration.packages
                if p.name == FluxBackend.name and p.namespace == namespace)
...
@router.get("/{namespace}/status", summary="trigger fetching status of flux-backend component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of flux-backend component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of flux-backend component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
