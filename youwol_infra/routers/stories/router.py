...
async def send_status(
    ...
    story = next(p for p in config.deployment_configuration.packages
                 if p.name == Stories.name and p.namespace == namespace)
...
@router.get("/{namespace}/status", summary="trigger fetching status of stories component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of stories component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
