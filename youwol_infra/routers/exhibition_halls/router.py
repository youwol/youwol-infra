...
@dataclass
class ExhibitionHalls(HelmPackage):

    name: str = "exhibition-halls"
    namespace: str = "prod"
...
async def send_status(
    ...
    halls = next(p for p in config.deployment_configuration.packages
                     if p.name == ExhibitionHalls.name and p.namespace == namespace)

    is_installed = await halls.is_installed()
    if not is_installed:
        resp = Status(
...
@router.get("/icon")
async def icon():
    path = Path(__file__).parent / 'exhibition-halls.jpg'
    return FileResponse(path)


@router.get("/{namespace}/status", summary="trigger fetching status of exhibition-halls component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of exhibition-halls component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of exhibition-halls component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
