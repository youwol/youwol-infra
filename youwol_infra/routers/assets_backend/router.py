...
@dataclass
class AssetsBackend(HelmPackage):

    name: str = "assets-backend"
    namespace: str = "prod"
    icon: str = "/api/youwol-infra/assets-backend/icon"

    chart_folder: Path = Configuration.services_folder / "assets-backend" / "chart"
...
async def send_status(
    ...
    assets = next(p for p in config.deployment_configuration.packages
                  if p.name == AssetsBackend.name and p.namespace == namespace)
...
@router.get("/icon")
async def icon():
    path = Path(__file__).parent / 'assets.jpeg'
    return FileResponse(path)


@router.get("/{namespace}/status", summary="trigger fetching status of assets-backend component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of assets-backend component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of assets-backend component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
