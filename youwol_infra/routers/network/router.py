...
async def send_status(
    ...
    network = next(p for p in config.deployment_configuration.packages
                   if p.name == Network.name and p.namespace == namespace)
...
@router.get("/{namespace}/status", summary="trigger fetching status of Network component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of Network component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of Network component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
