...
@dataclass
class CdnAppsServer(HelmPackage):

    name: str = "cdn-apps-server"
    namespace: str = "prod"
    icon: str = "/api/youwol-infra/cdn-apps-server/icon"

    chart_folder: Path = Configuration.services_folder / "cdn-apps-server" / "chart"
...
async def send_status(
    ...
    assets = next(p for p in config.deployment_configuration.packages
                  if p.name == CdnAppsServer.name and p.namespace == namespace)

    is_installed = await assets.is_installed()
    if not is_installed:
        resp = Status(
...
@router.get("/{namespace}/status", summary="trigger fetching status of cdn-apps-server component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of cdn-apps-server component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of cdn-apps-server component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
