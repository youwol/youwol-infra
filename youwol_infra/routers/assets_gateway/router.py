...
@dataclass
class AssetsGateway(HelmPackage):

    name: str = "assets-gateway"
    namespace: str = "prod"
...
async def send_status(
    ...
    assets = next(p for p in config.deployment_configuration.packages
                  if p.name == AssetsGateway.name and p.namespace == namespace)
...
@router.get("/{namespace}/status", summary="trigger fetching status of assets-gateway component")
async def status(
        request: Request,
        namespace: str,
...
@router.post("/{namespace}/install", summary="trigger install of assets-gateway component")
async def install(
        request: Request,
        namespace: str,
        body: HelmValues,
...
@router.post("/{namespace}/upgrade", summary="trigger upgrade of assets-gateway component")
async def upgrade(
        request: Request,
        namespace: str,
        body: HelmValues,
